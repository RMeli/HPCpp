...
int main([[maybe_unused]] int argc, [[maybe_unused]] char **argv) {
  ...
  if (mpi_size < (nprow * npcol)) {
    if (mpi_rank == 0)
      std::cout << "Error: mpi_size (" << mpi_size
                << ") is smaller than the requested grid size (" << nprow
                << " x " << npcol << ")" << std::endl;
  ...
  Cblacs_get(ic, what,
             &icontxt); /// get a value for nvpl_internal default, ic{-1} is not
                        /// used, what{0} is system default context

  nvpl_int_t myrow{0}, mycol{0};
  char grid_layout('C');
  Cblacs_gridinit(&icontxt, &grid_layout, nprow, npcol); /// create process grid
  Cblacs_gridinfo(icontxt, &nprow, &npcol, &myrow,
                  &mycol); /// set process grid with input icontxt
  ...
  if (icontxt < 0) {
    ...
    try {
      ...
      descinit_(descA, &m, &k, &mb, &mb, &izero, &izero, &icontxt, &lddA,
                &info);
      if (info) {
        throw std::runtime_error("Error: failed to descinit on matrix A");
      }
      descinit_(descB, &k, &n, &mb, &mb, &izero, &izero, &icontxt, &lddB,
                &info);
      if (info) {
        throw std::runtime_error("Error: failed to descinit on matrix B");
      }
      descinit_(descC, &m, &n, &mb, &mb, &izero, &izero, &icontxt, &lddC,
                &info);
      if (info) {
      ...
      char barrier_scope_all('A');
      {
        ...
        pdgemm_(&transA, &transB, &m, &n, &k, &alpha, A.data(), &ione, &ione,
                descA, B.data(), &ione, &ione, descB, &beta, C.data(), &ione,
                &ione, descC);
        Cblacs_barrier(icontxt, &barrier_scope_all);
        clock_gettime(CLOCK_MONOTONIC, &ts_end);
        t = ((static_cast<double>(ts_end.tv_sec) -
              static_cast<double>(ts_beg.tv_sec)) +
             (static_cast<double>(ts_end.tv_nsec) -
              static_cast<double>(ts_beg.tv_nsec)) /
                 billion);
      }
      if (mpi_rank == 0) {
